...
func completionCreate(w http.ResponseWriter, r *http.Request, llamaInstance *llamacpp.Llama) error {
	...
	var err error
	var result *schema.CompletionResponse

	// Execute completion
	if stream != nil {
		result, err = llamaInstance.Complete(r.Context(), req, func(chunk schema.CompletionChunk) error {
			stream.Write(schema.CompletionStreamDeltaType, chunk)
			return nil
		})
	} else {
		result, err = llamaInstance.Complete(r.Context(), req, nil)
	}
