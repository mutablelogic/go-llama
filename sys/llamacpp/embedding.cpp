...
// Forward declaration from context.cpp
...
void llama_go_set_embeddings(void* ctx_handle, bool embeddings) {
    ...
    CachedContext* cached = static_cast<CachedContext*>(ctx_handle);
    if (cached->ctx == nullptr) {
        llama_go_set_error("set_embeddings: context is null");
        return;
    }

    llama_set_embeddings(cached->ctx, embeddings);
}
...
llama_go_pooling_type llama_go_get_pooling_type(void* ctx_handle) {
    ...
    enum llama_pooling_type ptype = ::llama_pooling_type(cached->ctx);
...
float* llama_go_get_all_embeddings(void* ctx_handle) {
    ...
    float* embd = llama_get_embeddings(cached->ctx);
    if (embd == nullptr) {
        llama_go_set_error("get_all_embeddings: no embeddings available (ensure embeddings mode is enabled and batch was decoded)");
    }
    return embd;
}
...
float* llama_go_get_embeddings_seq(void* ctx_handle, int32_t seq_id) {
    ...
    float* embd = llama_get_embeddings_seq(cached->ctx, seq_id);
    if (embd == nullptr) {
        llama_go_set_error("get_embeddings_seq: no embeddings for sequence (ensure pooling_type != NONE)");
    }
    return embd;
}
