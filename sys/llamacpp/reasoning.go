...
// IsThinkingModel checks if the model's chat template suggests it's a reasoning model
// by looking for common thinking-related patterns in the template
func (m *Model) IsThinkingModel() bool {
	...
	// Also check model name/architecture for known reasoning models
	arch := m.Arch()
	name := m.Name()
	combined := strings.ToLower(arch + " " + name)
