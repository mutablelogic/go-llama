...
// Forward declaration from model.cpp
struct CachedModel {
...
bool llama_go_get_model_info(void* model_handle, llama_go_model_info* info) {
    ...
    CachedModel* cached = static_cast<CachedModel*>(model_handle);
    if (cached->model == nullptr) {
        llama_go_set_error("get_model_info: model is null");
        return false;
    }

    llama_model* model = cached->model;
...
int32_t llama_go_model_desc(void* model_handle, char* buf, size_t buf_size) {
    ...
    return llama_model_desc(cached->model, buf, buf_size);
}
